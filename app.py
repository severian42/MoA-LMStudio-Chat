    ...
    for chunk in output:
        if isinstance(chunk, dict):
            if 'content' in chunk and chunk['content'] is not None:
                ...
                if 'text' in chunk['choices'][0] and chunk['choices'][0]['text'] is not None:
                    bot_message += chunk['choices'][0]['text']
                elif 'delta' in chunk['choices'][0] and 'content' in chunk['choices'][0]['delta']:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                with gr.Accordion("Generation Parameters", open=True):
                    ...
                    max_tokens = gr.Slider(minimum=1, maximum=8092, step=1, value=int(MAX_TOKENS), label="Max Tokens")
